...
namespace gazebo
{
  class GZ_PLUGIN_VISIBLE ActorPlugin : public ModelPlugin
  {
    ...
    /// \brief Helper function to find index in target_list of current target
    private: int find_index();
    ...
    /// \brief Min x coordinate
    private: double min_x = 1.0;

    /// \brief Max x coordinate
    private: double max_x = 1.0;

    /// \brief Min y coordinate
    private: double min_y = 1.0;

    /// \brief Max y coordinate
    ...
    /// \brief List of target locations.
    private: std::vector<ignition::math::Vector3d> target_list;

    /// \brief Restart from origin flag. Used for loop the trajectory from the initial target
    private: bool loop_traj;
